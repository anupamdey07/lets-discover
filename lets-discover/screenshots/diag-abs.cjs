...
const abs = await page.evaluate(() => {
  ...
  return Array.from(ff.children).filter(el => getComputedStyle(el).position === 'absolute').map(el => {
    const cs = getComputedStyle(el);
    return { tag: el.tagName, cls: el.className, pos: cs.position, display: cs.display, inset: `${cs.top} ${cs.right} ${cs.bottom} ${cs.left}`, width: cs.width, height: cs.height, zIndex: cs.zIndex };
  });
});
console.log('Absolute children:', JSON.stringify(abs, null, 1));
// Test removing particles only
...
await sleep(50);
const afterPart = await page.evaluate(() => document.querySelector('.front-chat').getBoundingClientRect().top);
console.log(`before: ${before}, after removing particles: ${afterPart}`);
await browser.close();
