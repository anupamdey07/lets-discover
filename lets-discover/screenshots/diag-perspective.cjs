...
// disable perspective temporarily
await page.evaluate(() => {
  document.querySelector('.flip-container').style.perspective = 'none';
  document.querySelector('.flip-card').style.transformStyle = 'flat';
});
await sleep(100);
const after = await page.evaluate(() => document.querySelector('.front-face').getBoundingClientRect().top);
console.log(`Before: ${before}, After (perspective removed): ${after}`);
await browser.close();
