...
const metrics = await page.evaluate(() => {
  ...
  const bubble = document.querySelector('.front-bubble');
  const fb = document.querySelector('.fluid-bg').getBoundingClientRect();
  const sheet = document.querySelector('.persona-sheet');
  return {
    frontFace: { top: ff.top, bottom: ff.bottom, h: ff.height },
    ...
    fluidBg: { top: fb.top, left: fb.left, width: fb.width, height: fb.height },
    sheetClass: sheet?.className,
    sheetBg: sheet ? getComputedStyle(sheet).background : null,
    sheetBackdrop: sheet ? getComputedStyle(sheet).backdropFilter : null,
    weatherMood: document.querySelector('.weather-bg')?.className,
  };
});
