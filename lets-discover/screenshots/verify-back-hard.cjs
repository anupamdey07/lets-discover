...
// FRESH load → directly to back via button → hard flick back->front
...
await page.click('.front-explore-btn'); await sleep(1000);
console.log('on back:', await page.evaluate(() => document.querySelector('.flip-back').classList.contains('flip-visible')));
console.log('elem@(8,cy):', await page.evaluate(([x,y]) => { const e = document.elementFromPoint(x,y); return e ? e.tagName+'.'+(e.className||'') : 'null'; }, [8, b.y+b.h/2]));
await page.evaluate(() => { window.__backDown = null; window.__flipDbg = null; });
const cy = b.y + b.h/2;
await page.mouse.move(8, cy); await page.mouse.down();
for (let i = 1; i <= 10; i++) { await page.mouse.move(8 + i * 10, cy); await sleep(8); }
await page.mouse.up();
const s = await sampleSpring(-1);
console.log('backDown:', await page.evaluate(() => window.__backDown));
