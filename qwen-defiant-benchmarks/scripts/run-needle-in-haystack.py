#!/usr/bin/env python3
"""
Needle In Haystack benchmark for Qwen-Defiant-9B.
Tests long-context retrieval and attention mechanisms.
"""
...
RESULTS_DIR = PROJECT_ROOT / "results" / "Needle_In_Haystack"
...
def load_dataset() -> List[Dict[str, str]]:
    print("Loading Needle In Haystack dataset...")
    data_file = PROJECT_ROOT / "datasets" / "needle-in-haystack-test.jsonl"
...
def call_model(problem: Dict[str, str]) -> str:
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a precision retrieval assistant. Read the context carefully and extract ONLY the requested information."
            },
            {
                "role": "user",
                "content": prompt
    ...
    test_id = problem.get("id", f"NIH-{len(samples)}")
...
def evaluate_generated_answer(problem: Dict[str, str], generated_answer: str) -> bool:
    ...
    if normalized_generated == normalized_correct or str(correct_answer).lower() in normalized_generated:
        print(f"  PASS - Found: {generated_answer}")
        return True
    
    print(f"  FAIL - Expected: {correct_answer}, Got: {generated_answer[:80]}")
    return False


def run_benchmark() -> Dict[str, Any]:
    print("=" * 60)
    print("Needle In Haystack Benchmark - Qwen-Defiant-9B")
    print("=" * 60)
    ...
    for i, problem in enumerate(problems, 1):
        test_id = problem.get("id", f"NIH-{i}")
        ...
        sample = {
            "id": problem.get("id", f"NIH-{i}"),
            "context_length": len(problem.get("context", "")),
...
def update_dashboard(results: Dict[str, Any]) -> None:
    ...
    # Remove existing Needle_In_Haystack rows
    content = content.replace('<tr class="benchmark Needle_In_Haystack">', '')
    ...
    new_row = f'''<tr class="benchmark Needle_In_Haystack">
